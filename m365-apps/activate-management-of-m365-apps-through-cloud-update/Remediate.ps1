...
# Description:         Make sure device is locked to M365 Apps Cloud Update. If onboarded but not locked, remediation starts and sets a registry key to lock it.
...
try {
    ...
    if (-not $cloudUpdateKeyExists) {
        ...
        if ($cloudUpdateValueIs -eq $cloudUpdateValueShould) {
            ...
            Write-Host "Machine was onboarded but not LOCKED to Cloud Update - Remediated."
        }
    }
